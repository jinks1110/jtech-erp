...
export default function QuotationDetailPage() {
  ...
  const handleCopyQuotation = async () => {
    if (!window.confirm('이 견적서를 똑같이 복사하여 새 견적서를 작성하시겠습니까?\n(견적일자는 오늘 날짜로 자동 세팅됩니다.)')) return;
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-100 min-h-screen text-black print:bg-white print:p-0">
      ...
      {/* 상단 컨트롤 버튼 (모바일 화면에 맞춰 버튼 배열 최적화) */}
      ...
      {isEditing ? (
        <div className="max-w-4xl mx-auto bg-white p-4 md:p-8 shadow-lg border-2 border-yellow-400 rounded-lg no-print">
                ...
                {editItems.map((item, idx) => (
                  <tr key={idx} className="text-sm">
                    <td className="border p-2">
                      <select className="w-full outline-none bg-transparent" value={item.product_id || ''} onChange={(e) => handleProductSelect(idx, e.target.value)}>
          ...
          <div className="md:hidden space-y-4 mb-4">
            {editItems.map((item, idx) => (
              <div key={idx} className="bg-white border-2 border-yellow-200 rounded-xl p-4 shadow-sm relative">
                {editItems.length > 1 && (
                  <button onClick={() => removeEditItem(idx)} className="absolute top-3 right-3 text-red-500 bg-red-50 w-8 h-8 rounded-full font-bold flex items-center justify-center">X</button>
                )}
                ...
                <div className="mb-3">
                  ...
                  <input type="text" className="w-full border rounded-lg p-2.5 outline-none focus:border-yellow-500 text-base" value={item.name} onChange={(e) => { const newItems = [...editItems]; newItems[idx].name = e.target.value; setEditItems(newItems); }} />
                </div>
                <div className="flex gap-3 mb-2">
                  <div className="flex-1">
                    <label className="text-xs font-bold text-gray-500 block mb-1">수량</label>
                    <input type="number" className="w-full border rounded-lg p-2.5 outline-none focus:border-yellow-500 text-center text-base" value={item.qty === 0 ? '' : item.qty} onChange={(e) => { const newItems = [...editItems]; newItems[idx].qty = Number(e.target.value); setEditItems(newItems); }} />
                  </div>
                  <div className="flex-1">
                    <label className="text-xs font-bold text-gray-500 block mb-1">단가 (원)</label>
                    <input type="number" className="w-full border rounded-lg p-2.5 outline-none focus:border-yellow-500 text-right text-base" value={item.price === 0 ? '' : item.price} onChange={(e) => { const newItems = [...editItems]; newItems[idx].price = Number(e.target.value); setEditItems(newItems); }} />
                  </div>
                </div>
              </div>
            ))}
        ...
        /* === 추가: A4 뷰어도 모바일에서 가로 스크롤 가능하게 랩핑 (overflow-x-auto) === */
